...
export default function Admin() {
  ...
  const handleCreateStaff = () => {
    ...
    if (newStaffPassword !== newStaffPasswordConfirm) {
      toast.error("パスワードが一致しません"); return;
    }
    if (newStaffPassword.length < 8) {
      toast.error("パスワードは8文字以上で設定してください"); return;
    }
  ...
  return (
    <div style={{ minHeight: "100dvh", background: "#faf8f5", fontFamily: "'Noto Sans JP', sans-serif" }}>
      ...
      <main style={{ maxWidth: "1200px", margin: "0 auto", padding: "2rem 1.5rem" }}>
        ...
        {/* スタッフアカウント管理セクション（Manus管理者のみ表示） */}
        {isManusAuth && (
          <div style={{ marginTop: "3rem", borderTop: "1px solid #e8ddd0", paddingTop: "2rem" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1rem", flexWrap: "wrap", gap: "0.5rem" }}>
              <h2 style={{ fontSize: "1rem", fontWeight: 700, color: "#2a1a0a", margin: 0 }}>スタッフアカウント管理</h2>
              <button
            ...
            {/* スタッフ作成フォーム */}
            {showCreateStaff && (
              <div style={{ background: "white", border: "1px solid #e8ddd0", borderRadius: "6px", padding: "1.5rem", maxWidth: "480px", marginBottom: "1.5rem" }}>
            ...
            {staffList && staffList.length === 0 && (
              <p style={{ fontSize: "0.82rem", color: "#aaa", textAlign: "center", padding: "1.5rem" }}>スタッフアカウントがまだ登録されていません。上の「＋スタッフを追加」ボタンから作成してください。</p>
            )}

            {/* コンテンツ管理リンク */}
            <div style={{ marginTop: "1.5rem", display: "flex", flexWrap: "wrap", gap: "0.75rem" }}>
              {[
                { label: "ビフォーアフター管理", href: "/admin/before-after" },
                { label: "お客様の声管理", href: "/admin/testimonials" },
                { label: "ブログ管理", href: "/admin/blog" },
                { label: "メニュー・料金管理", href: "/admin/menus" },
                { label: "サロン管理", href: "/admin/salons" },
              ].map(({ label, href }) => (
                <a
                  key={href}
                  href={href}
                  style={{ padding: "0.5rem 1.25rem", background: "#faf8f5", color: "#5a3e28", border: "1px solid #d4c5b0", borderRadius: "4px", textDecoration: "none", fontSize: "0.82rem", fontWeight: 600, letterSpacing: "0.03em" }}
                >
                  {label}
                </a>
              ))}
