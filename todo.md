...
## 全国認定サロンリストの再表示（2026-09-16）

- [x] 以前の全国サロンリスト実装・掲載データ・ルートを確認する（/salonsは既存、公開データは0件のため直営店情報から再登録）
- [x] 実在・掲載可否を確認できる認定サロンのみを再表示する（THE HERBS直営2店舗を公開）
- [x] サロンリストの表示を検証し、チェックポイントを保存する（2店舗・地図マーカー・絞り込み・トップページ導線を確認）
- [x] THE HERBS植物美容サロンとTHE HERBS神戸阪急店を、提供情報に基づき公開認定サロンとして登録する

## コライユ メニュー紹介（2026-09-16）

- [x] 既存のメニュー・予約導線を確認し、コライユの掲載箇所と必要情報を整理する
- [x] コライユをヘアカラー・パーマ後の頭皮コンディショニングメニューとしてサイトに追加する
- [x] 薬機法・景品表示法に配慮したコライユの紹介コンテンツを掲載する
- [x] 表示・予約導線を検証し、チェックポイントを保存する（コライユは両店舗向け予約メニューAPIで表示確認）
- [x] コライユを約5分のカラーパーマ用オプションとして掲載し、料金非掲載・神戸阪急店はコライユミストのみと明記する
- [x] ヴェルデをコライユと分け、頭皮と髪の状態に合わせるパーソナル頭皮ケアとして独立紹介する
- [x] service_menusにコライユを店舗別オプションとして登録し、料金非掲載表示に対応する
- [ ] ヴェルデの提供店舗・所要時間を確認後、service_menusへ登録する

## パートナーサロン一覧へのメニュー掲載（2026-09-16）

- [x] トップページのコライユ・ヴェルデ共通紹介を維持する
- [x] THE HERBS植物美容サロンの詳細に、コライユとヴェルデの紹介を掲載する
- [x] THE HERBS神戸阪急店の詳細に、コライユミストの紹介を掲載する
- [x] /salonsで各店舗の詳細を個別に表示し、メニュー情報をデスクトップ・モバイルで検証する
- [x] /salonsのURLパラメーターで各店舗詳細を直接開けるようにする

## THE HERBS メソッド紹介への再構成（2026-09-16）

- [x] パートナーサロン一覧からコライユ・ヴェルデの店舗固有表示を外す
- [x] トップページのコライユ・ヴェルデ紹介を全店舗共通のTHE HERBSメソッドとして再構成する
- [x] コライユ・ヴェルデの資料画像を用い、メソッドの役割と流れを詳しく掲載する
- [x] 表示・表現を検証し、チェックポイントを保存する（トップページのデスクトップ・モバイル、サロン一覧の店舗固有表示削除を確認）

## アプリ向けサロン検索ページの拡張（2026-09-16）

- [x] 全店舗の対応メニューとして「コライユ（デトックス）」「ヴェルデ（パーソナルケア）」を共通メソッド定義として検索対象に追加する
- [x] /salonsの対応メニュー絞り込みにコライユとヴェルデを追加する
- [x] /salonsに全店舗共通のTHE HERBSメソッドを画像付きで掲載する
- [x] アプリ経由を想定したデスクトップ・モバイルの検索表示を検証し、チェックポイントを保存する（コライユ・ヴェルデのURL指定絞り込みを確認）
- [x] /salonsのURLパラメーターでコライユ・ヴェルデの対応メニュー絞り込みを直接指定できるようにする

## THE HERBSメソッド画像の調整（2026-09-16）

- [x] コライユ・ヴェルデの資料図版を外し、切れのないハーブ画像のみを表示する
- [x] デスクトップ・モバイルで画像の見え方を確認し、チェックポイントを保存する

## 取扱サロン案内文の更新（2026-09-16）

- [x] /salonsの案内文を「THE HERBS 取扱サロン」向けの指定文に更新する
- [ ] 表示を確認し、チェックポイントを保存する
